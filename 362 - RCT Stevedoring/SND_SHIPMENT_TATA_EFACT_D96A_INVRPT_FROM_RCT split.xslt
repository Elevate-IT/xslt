...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--  Maximum LINLoop1 segments per output EFACT_D96A_INVRPT message.
       Pass a different value externally to change the split size.   -->
  <xsl:param name="MaxLinCount" as="xs:integer" select="110"/>

  <!--  Key used to group Carrier nodes that share the same Attribute03/Attribute02 combination  -->
  ...
  <xsl:template match="//s0:Message/s0:Documents/s0:Document">
    ...
    <xsl:variable name="loop1Items"
      select="s0:Carriers/s0:Carrier[s0:Contents/s0:Content/s0:Posted = '1']
                [count(. | key('GroupedLIN', concat(s0:Contents/s0:Content/s0:Attribute03, '-', s0:Contents/s0:Content/s0:Attribute02))[1]) = 1]
                [concat(s0:Contents/s0:Content/s0:Attribute03, '-', s0:Contents/s0:Content/s0:Attribute02) != '-']"/>
    <!--  Loop-2 source nodes: Content items without Attribute03/Attribute02  -->
    <xsl:variable name="loop2Items"
      select="s0:Carriers/s0:Carrier/s0:Contents/s0:Content[s0:Posted = '1'][count(s0:Attribute03) = 0][count(s0:Attribute02) = 0]"/>
    <!--  Combined list: Carrier nodes first, then Content nodes  -->
    <xsl:variable name="allLINItems" select="$loop1Items | $loop2Items"/>
      ...
      <xsl:for-each-group select="$allLINItems" group-by="ceiling(position() div $MaxLinCount)">
        ...
        <Message Iteration="{current-grouping-key()}">
          <ns0:EFACT_D96A_INVRPT>
            ...
            <!--  Iterate over items in this batch  -->
            <xsl:for-each select="$batchItems">
              <xsl:choose>
                <!--  Loop-1 item: node is a s0:Carrier (grouped by Attribute03/Attribute02)  -->
                <xsl:when test="self::s0:Carrier">
                      ...
                      <ns0:RFFLoop4>
                        <ns0:RFF_4>
                          <ns0:C506_4>
                            <C50601>AAK</C50601>
                            <C50602>
                              <xsl:value-of select="translate((//s0:No)[1], translate((//s0:No)[1], '0123456789', ''), '')"/>
                            </C50602>
                          </ns0:C506_4>
                        </ns0:RFF_4>
                ...
                <!--  Loop-2 item: node is a s0:Content (no Attribute03/Attribute02)  -->
                <xsl:otherwise>
