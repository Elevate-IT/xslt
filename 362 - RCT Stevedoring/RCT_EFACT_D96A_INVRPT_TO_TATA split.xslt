<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <xsl:key name="Group-By-Item-CorusNo" match="//s0:CarrierContent" use="concat(s0:Attribute03, '-', s0:Attribute02)"/>
  ...
  <xsl:template match="//s0:Message/s0:Customers/s0:Customer">
    <!--  Collect all unique grouped CarrierContent items that will become LIN segments  -->
    <xsl:variable name="allLINItems" select="//s0:CarrierContent [count(. | key('Group-By-Item-CorusNo', concat(s0:Attribute03, '-', s0:Attribute02))[1]) = 1] [concat(s0:Attribute03, '-', s0:Attribute02) != '-']"/>
    <!-- 
         Emit one EFACT_D96A_INVRPT per chunk of $MaxLinCount items.
         ceiling(position() div $MaxLinCount) maps:
         items 1..$MaxLinCount           → group-key 1  (file 1)
         items $MaxLinCount+1..2×$Max    → group-key 2  (file 2)
    ...
    <StockStatusEnvelope>
      <xsl:for-each-group select="$allLINItems" group-by="ceiling(position() div $MaxLinCount)">
        ...
        <Message Iteration="{ceiling(position() div $MaxLinCount)}">
        <ns0:EFACT_D96A_INVRPT>
          ...
          <!--  ── LIN loop: iterate over this batch's CarrierContent items ──  -->
