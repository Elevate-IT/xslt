<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:s0="www.boltrics.nl/sendtrip:v1.00"
                xmlns:MyScript="http://schemas.microsoft.com/BizTalk/2003/MyScript"
                exclude-result-prefixes="msxsl s0 MyScript" version="1.0">
  <xsl:output omit-xml-declaration="yes" method="xml" version="1.0" />
  <xsl:key name="Group-by-CarrierQty" match="//s0:Carrier" use="s0:Contents/s0:Content/s0:Quantity" />
  <xsl:template match="/">
    <xsl:apply-templates select="//s0:Message/s0:Trips/s0:Trip" />
  </xsl:template>
  <xsl:template match="//s0:Message/s0:Trips/s0:Trip">
    <DESPATCH-ACK>
      <UNH>
      ...
      <xsl:for-each select="s0:TripLines/s0:TripLine/s0:Documents/s0:Document/s0:DocumentLines/s0:DocumentLine[s0:Type = 1]">
        ...
        <DELIVERY>
          ...
          <DespatchData>
            ...
            <CarrierRunId>
              <xsl:value-of select="../../s0:TripNo" />
            </CarrierRunId>
            <CarrierRunSeqId>
              <xsl:value-of select="s0:Attributes/s0:Attribute[s0:Code = 'SK_CR_SEQ']/s0:Value" />
            </CarrierRunSeqId>
            <CarrierRunDate>
              <xsl:value-of select="MyScript:ParseDate(//s0:Trip/s0:Date, 'yyyy-MM-dd', 'yyyyMMdd')" />
            </CarrierRunDate>
            <CarrierShipmentReference>
              <xsl:value-of select="../../s0:No" />
            </CarrierShipmentReference>
            <xsl:for-each select="../../s0:Carriers/s0:Carrier[s0:Contents/s0:Content/s0:DocumentLineNo = $DocLineNo][count(. | key('Group-by-CarrierQty', s0:Contents/s0:Content/s0:Quantity)[1]) = 1]">
              <xsl:variable name="LineKey" select="s0:Contents/s0:Content/s0:Quantity" />
              <xsl:if test="s0:Contents/s0:Content/s0:Quantity != ''">
                ...
                <PalletsDespatch>
                  ...
                  <NumberOfPallets>
                    <xsl:value-of select="count(key('Group-by-CarrierQty',$LineKey)/s0:No)" />
                  </NumberOfPallets>
                  <NumberOfPiecesPerPallet>
                    <xsl:value-of select="key('Group-by-CarrierQty',$LineKey)/s0:Contents/s0:Content/s0:Quantity" />
                  </NumberOfPiecesPerPallet>
                  <xsl:for-each select="key('Group-by-CarrierQty',$LineKey)/s0:No">
                    <PalletSSCC>
                      <PalletSSCCCode>
