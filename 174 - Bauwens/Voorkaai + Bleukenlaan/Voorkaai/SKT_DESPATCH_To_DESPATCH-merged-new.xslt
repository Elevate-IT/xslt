...
>
  ...
  <xsl:key name="Group-by-DelAddress_PickingInstructions" match="//DELIVERY" use="concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions)" />
  ...
  <xsl:template match="UNH">
    ...
    <xsl:for-each select="//DELIVERY[count(. | key('Group-by-DelAddress_PickingInstructions', concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions))[1]) = 1]">
      <xsl:variable name="LineKey" select="concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions)" />
      <xsl:if test="concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions) != '-'">
        <xsl:value-of select="MyScript:SetMinRunSeqId('0')" />
        <xsl:for-each select="key('Group-by-DelAddress_PickingInstructions', concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions))/DespatchData">
    ...
    <xsl:for-each select="//DELIVERY[count(. | key('Group-by-DelAddress_PickingInstructions', concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions))[1]) = 1]">
      <xsl:variable name="LineKey" select="concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions)" />
      <xsl:variable name="DelAddrId" select="CallOffData/DeliveryAddressId"/>
      <xsl:variable name="PickInstr" select="DespatchData/PickingInstructions"/>
      ...
      <xsl:if test="concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions) != '-'">
        ...
        <xsl:copy>
          <xsl:for-each select="//CallOffData[DeliveryAddressId = $DelAddrId][../DespatchData/PickingInstructions = $PickInstr]">
            <xsl:variable name="Counter2" select="MyScript:GetCounter2(1)"/>
            <xsl:variable name="LoadSeq" select="MyScript:GetLoadSeq(count(//DELIVERY[count(. | key('Group-by-DelAddress_PickingInstructions', concat(CallOffData/DeliveryAddressId,'-', DespatchData/PickingInstructions))[1]) = 1]))"/>
            <xsl:copy>
              <DeliveryId>
