          ...
          <xsl:if test="concat(s0:DocumentLineNo,'-',s0:ExternalBatchNo) != '-'">
            <ttdesp:DeliveryLine>
              <ttdesp:DeliveryLineID schemeID="REF">
                <xsl:choose>
                  <xsl:when test="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:Attributes/s0:Attribute[s0:Code='LINENO']/s0:Value != ''">
              ...
              <ttdesp:ProductUnit>
                <ttdesp:ProductUnitID_GTIN schemeID="GTIN">
                  <xsl:choose>
                    <xsl:when test="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:Attributes/s0:Attribute[s0:Code = 'GTIN']/s0:Value != ''">
                      <xsl:value-of select="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:Attributes/s0:Attribute[s0:Code = 'GTIN']/s0:Value" />
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:choose>
                        <xsl:when test="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:EANCode != ''">
                          <xsl:value-of select="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:EANCode" />
                        </xsl:when>
                        <xsl:otherwise>
                          <xsl:value-of select="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:ExternalNo" />
                        </xsl:otherwise>
                      </xsl:choose>
                    </xsl:otherwise>
                  </xsl:choose>
                ...
                <!-- = LogisticUnit-->
                <ttdesp:BatchID>
                  <xsl:value-of select="key('Group-by-LineNo-BatchNo',$LineKey)/s0:ExternalBatchNo" />
                </ttdesp:BatchID>
                <ttdesp:ProductUnitName>
                  <xsl:value-of select="key('Group-by-LineNo-BatchNo',$LineKey)/../../s0:Description" />
                </ttdesp:ProductUnitName>
              </ttdesp:ProductUnit>
            </ttdesp:DeliveryLine>
          </xsl:if>
