<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="//s0:Message/s0:CarrierStatusChanges" />
  </xsl:template>
  <xsl:template match="//s0:Message/s0:CarrierStatusChanges">
    <ns0:EFACT_D96A_INVRPT>
      ...
      <xsl:for-each select="s0:CarrierStatusChange[MyScript:ConvertStatusCode(s0:NewStatusCode) != MyScript:ConvertStatusCode(s0:OldStatusCode)]">
        <ns0:LINLoop1>
          <ns0:LIN>
            <LIN01>
          ...
          <ns0:QTYLoop2>
            <ns0:QTY_2>
              <ns0:C186_2>
                <C18601>
                  <xsl:choose>
                    <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'UN'">05</xsl:when>
                    <xsl:otherwise>55</xsl:otherwise>
                  </xsl:choose>
                </C18601>
                <C18602>
                  <xsl:value-of select="MyScript:Abs(s0:ContentLines/s0:CarrierContent/s0:Quantity)" />
                </C18602>
                <C18603>CS</C18603>
              </ns0:C186_2>
            </ns0:QTY_2>
            <ns0:INV>
              <INV01>
                <xsl:choose>
                  <xsl:when test="MyScript:ConvertStatusCode(s0:OldStatusCode) = 'QI'">
                    <xsl:choose>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'QH'">IB</xsl:when>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'UN'">QI</xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="MyScript:ConvertStatusCode(s0:NewStatusCode)" />
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:when>
                  <xsl:when test="MyScript:ConvertStatusCode(s0:OldStatusCode) = 'QH'">
                    <xsl:choose>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'QI'">IB</xsl:when>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'UN'">QH</xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="MyScript:ConvertStatusCode(s0:NewStatusCode)" />
                      </xsl:otherwise>
                    </xsl:choose>
              ...
              <xsl:choose>
                <xsl:when test="s0:ReasonCode != ''">
                  <INV03>
                    <xsl:value-of select="s0:ReasonCode" />
                  </INV03>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:choose>
                    <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'QH'">
                      <INV03>933</INV03>
                    </xsl:when>
                    <xsl:when test="MyScript:ConvertStatusCode(s0:OldStatusCode) = 'QH'">
                      <INV03>933</INV03>
