<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="//s0:Message/s0:CarrierStatusChanges/s0:CarrierStatusChange" />
  </xsl:template>
  <xsl:template match="//s0:Message/s0:CarrierStatusChanges/s0:CarrierStatusChange">
    <ns0:EFACT_D16B_INVRPT>
      ...
      <ns0:FTX>
        <FTX01>AAI</FTX01>
        <ns0:C108>
          <C10801>CARRIER STATUS CHANGE</C10801>
        </ns0:C108>
        <FTX05>EN</FTX05>
      </ns0:FTX>
      <ns0:DTM>
      ...
      <ns0:DTM>
        <ns0:C507>
          <C50701>334</C50701>
          <C50702>
            <xsl:value-of select="MyScript:ParseDate(concat(s0:LogDate,'T',s0:LogTime), 's', 'yyyyMMddHHmm')" />
          </C50702>
          <C50703>203</C50703>
        </ns0:C507>
      </ns0:DTM>
      <ns0:RFFLoop1>
        <ns0:RFF>
          <ns0:C506>
            <C50601>HN</C50601>
            <C50602>
              <xsl:value-of select="s0:EntryNo" />
            </C50602>
          </ns0:C506>
          </ns0:RFF>
      </ns0:RFFLoop1>
        ...
        <ns0:INVLoop1>
          <ns0:INV>
            <ns0:C522>
              <C52201>
                <xsl:choose>
                  <xsl:when test="MyScript:ConvertStatusCode(s0:OldStatusCode) = 'FREE'">
                    <xsl:choose>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'QUALITY'">322</xsl:when>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'BLOCKED'">344</xsl:when>
                    </xsl:choose>
                  </xsl:when>
                  <xsl:when test="MyScript:ConvertStatusCode(s0:OldStatusCode) = 'QUALITY'">
                    <xsl:choose>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'FREE'">321</xsl:when>
                      <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'BLOCKED'">350</xsl:when>
                    </xsl:choose>
                  </xsl:when>
                  <xsl:when test="MyScript:ConvertStatusCode(s0:OldStatusCode) = 'BLOCKED'">
                      <xsl:choose>
                        <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'FREE'">343</xsl:when>
                        <xsl:when test="MyScript:ConvertStatusCode(s0:NewStatusCode) = 'QUALITY'">349</xsl:when>
                      </xsl:choose>
                    </xsl:when>
                </xsl:choose>
              </C52201>
