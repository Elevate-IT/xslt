<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="//s0:Message/s0:Customers/s0:Customer/s0:Adjustments" />
  </xsl:template>
  <xsl:template match="//s0:Message/s0:Customers/s0:Customer/s0:Adjustments">
    <ns0:EFACT_D16B_INVRPT>
      ...
      <ns0:FTX>
        ...
        <ns0:C108>
          <C10801>
            <xsl:value-of select="MyScript:ToUpper(s0:Adjustment/s0:DocumentNo)" />
          </C10801>
        </ns0:C108>
        <FTX05>EN</FTX05>
      </ns0:FTX>
      ...
      <ns0:DTM>
        <ns0:C507>
          <C50701>334</C50701>
          <C50702>
            <xsl:value-of select="MyScript:ParseDate(s0:Adjustment/s0:Carrier/s0:ExecutionDateTime, 's', 'yyyyMMddHHmm')" />
          </C50702>
          <C50703>203</C50703>
        </ns0:C507>
      </ns0:DTM>
      <ns0:RFFLoop1>
        <ns0:RFF>
          <ns0:C506>
            <C50601>HN</C50601>
            <C50602>
              <xsl:value-of select="s0:Adjustment/s0:EntryNo" />
            </C50602>
          </ns0:C506>
        </ns0:RFF>
      </ns0:RFFLoop1>
      ...
      <xsl:for-each select="s0:Adjustment[s0:ReasonCode != '']">
        <ns0:LINLoop1>
          <ns0:LIN>
            <LIN01>
          ...
          <ns0:INVLoop1>
            <ns0:INV>
              <ns0:C522>
                <C52201>
                  <xsl:choose>
                    <xsl:when test="MyScript:ConvertStatusCode(s0:Carrier/s0:StatusCode) = 'FREE'">
                      <xsl:choose>
                        <xsl:when test="s0:ReasonCode = 'BQ'">551</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'BQR'">552</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DE'">555</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DER'">556</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'ED'">951</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'EDR'">952</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DI'">992</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DIR'">991</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'SI'">901</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'SIR'">902</xsl:when>
                      </xsl:choose>
                    </xsl:when>
                    <xsl:when test="MyScript:ConvertStatusCode(s0:Carrier/s0:StatusCode) = 'QUALITY'">
                      <xsl:choose>
                        <xsl:when test="s0:ReasonCode = 'BQ'">553</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'BQR'">554</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DE'">555</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DER'">556</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'ED'">953</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'EDR'">954</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DI'">992</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DIR'">991</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'SI'">901</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'SIR'">902</xsl:when>
                    ...
                    <xsl:when test="MyScript:ConvertStatusCode(s0:Carrier/s0:StatusCode) = 'BLOCKED'">
                      <xsl:choose>
                        <xsl:when test="s0:ReasonCode = 'BQ'">553</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'BQR'">554</xsl:when>
                        <xsl:when test="s0:ReasonCode = 'DE'">555</xsl:when>
                ...
                <C52203>
                  <xsl:choose>
                    <xsl:when test="s0:ReasonCode = 'SI' or s0:ReasonCode = 'SIR'">MB1C</xsl:when>
                    <xsl:otherwise>
                      <xsl:choose>
                        <xsl:when test="s0:ReasonCode = 'DI' or s0:ReasonCode = 'DIR'">MB1B</xsl:when>
                        <xsl:otherwise>MB1A</xsl:otherwise>
                      </xsl:choose>
                    </xsl:otherwise>
                  </xsl:choose>
                </C52203>
