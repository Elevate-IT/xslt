<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="envelope/purchase_order" />
  </xsl:template>
  ...
  <xsl:template name="getBatchNo">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>$stringContainingBatch</xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="envelope/purchase_order">
    <ns0:Message>
      <ns0:Header>
        <ns0:MessageID>
      ...
      <ns0:Documents>
        <ns0:Document>
          ...
          <ns0:DeliveryDate>
            <xsl:value-of select="MyScript:ParseDate(purchase_order_line/expected_delivery, 'yyyy-MM-ddThh:mm:ss.fff', 'yyyy-MM-dd')" />
          </ns0:DeliveryDate>
          <ns0:ExternalDocumentNo>
            <xsl:value-of select="purchase_order_nr" />
          </ns0:ExternalDocumentNo>
          <ns0:ExternalReference>
            <xsl:value-of select="container_reference" />
          </ns0:ExternalReference>
          <ns0:SenderAddress>
            <ns0:Name>
          ...
          <xsl:if test="count(purchase_order_line)&gt;0">
            <ns0:DocumentLines>
              <xsl:for-each select="purchase_order_line">
                <ns0:DocumentLine>
                  <ns0:ExternalNo>
                  ...
                  <xsl:if test="purchase_order_line_lot/purchase_order_line_lotnr_supplier != ''">
                    <ns0:ExternalBatchNo>
                      <!--<xsl:call-template name="getBatchNo">
                        <xsl:with-param name="stringContainingBatch">
                          <xsl:value-of select="purchase_order_line_lot/purchase_order_line_lotnr_supplier" />
                        </xsl:with-param>
                    ...
                    <xsl:if test="purchase_order_line_lot/purchase_order_line_best_before != ''">
                      <ns0:ExpirationDate>
                        <xsl:value-of select="purchase_order_line_lot/purchase_order_line_best_before"/>
                      </ns0:ExpirationDate>
                    </xsl:if>
                  </xsl:if>
                  <ns0:Attributes>
                    ...
                    <xsl:if test="purchase_order_line_lot/purchase_order_line_lotnr_internal != ''">
                      <ns0:Attribute>
                        <ns0:Code>EXTBATCH2</ns0:Code>
                        <ns0:Value>
                          <xsl:value-of select="purchase_order_line_lot/purchase_order_line_lotnr_internal" />
                        </ns0:Value>
                      </ns0:Attribute>
                    </xsl:if>
                  </ns0:Attributes>
