              ...
              <xsl:for-each select="DETAIL/ITEMLINE[count(. | key('Group-by-CUSTPRODCODE-LINENO-PROD-THT-BATCH',concat(CUSTPRODCODE, '-', LINENO, '-', ASKED/TRACKING/PRODUCTIONDATE,'-',ASKED/TRACKING/REQEXPDATE,'-',ASKED/TRACKING/REQLOT))[1]) = 1]">
                ...
                <xsl:if test="$LineKey != '--'">
                  <ns0:DocumentLine>
                    ...
                    <ns0:ExpirationDate>
                      <xsl:value-of select="format-date(xs:date(ASKED/TRACKING/REQEXPDATE), '[D01]-[M01]-[Y0001]')" />
                    </ns0:ExpirationDate>
                    <ns0:ProductionDate>
                      <xsl:value-of select="format-date(xs:date(ASKED/TRACKING/PRODUCTIONDATE), '[D01]-[M01]-[Y0001]')" />
                    </ns0:ProductionDate>
